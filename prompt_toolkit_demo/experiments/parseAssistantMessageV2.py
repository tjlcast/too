...
def parse_assistant_message_v2(
    ...
    # Index where the current text block started.
    current_text_content_start = 0
    current_text_content: Optional[TextContent] = None
    # Index *after* the opening tag of the current tool use.
    current_tool_use_start = 0
    current_tool_use: Optional[ToolUse] = None
    # Index *after* the opening tag of the current param.
    ...
    while i < length:
        ...
        if current_tool_use and current_param_name:
            ...
            if (current_char_index >= close_tag_len - 1 and
                ...
                value = assistant_message[
                    ...
                    # End before the closing tag.
                    current_char_index - close_tag_len + 1
                ]
        ...
        if current_tool_use and not current_param_name:
            ...
            for tag, param_name in tool_param_open_tags.items():
                ...
                if (current_char_index >= tag_len - 1 and
                    ...
                    # Value starts after the tag.
                    current_param_value_start = current_char_index + 1
            ...
            if (current_char_index >= tool_close_tag_len - 1 and
                ...
                tool_content_slice = assistant_message[
                    ...
                    # To before the tool closing tag.
                    current_char_index - tool_close_tag_len + 1
                ]
                ...
                # Potential text starts after this tag.
                current_text_content_start = current_char_index + 1
                i += 1
        ...
        if not current_tool_use:
                ...
                if (current_char_index >= tag_len - 1 and
                    ...
                    if current_text_content:
                        current_text_content.content = assistant_message[
                            ...
                            # To before the tool tag starts.
                            current_char_index - tag_len + 1
                        ].strip()

                        # Ended because tool started.
                        ...
                        potential_text = assistant_message[
                            # From where text *might* have started.
                            current_text_content_start:
                            # To before the tool tag starts.
                            current_char_index - tag_len + 1
                        ].strip()
                        ...
                        if potential_text:
                            content_blocks.append(
                                TextContent(potential_text, False))

                    # Start the new tool use.
                    current_tool_use = ToolUse(tool_name, True)
                    # Tool content starts after the opening tag.
                    current_tool_use_start = current_char_index + 1
            ...
            if not current_text_content:
                ...
                # Text starts at the current character.
                current_text_content_start = current_char_index
    ...
    if current_tool_use and current_param_name:
        # From param start to end of string.
        value = assistant_message[current_param_value_start:]
        # Don't trim content parameters to preserve newlines, but strip first and last newline only
        if current_param_name == "content":
            if value.startswith('\n'):
                value = value[1:]
...
def parse_assistant_message(
    ...
    if tool_param_names is None:
        tool_param_names = ["content", "path",
                            "rule_name", "search_query", "file_path"]
